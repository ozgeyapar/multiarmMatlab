function [ optbeta, kgifinal] = cKGstarUndisRatioCopy( parameters, mu, sigma, i )
%cKGstarUndisRatio
...
%%
    ...
    if Mprime == 1 %there is no value in sampling, by definition kg takes one sample
       ...
       for l = 1:Mprime-1
            var = varfixedbeta(optbeta, parameters.lambdav(i), efnsi );
            temp(l)=(b(l+1)-b(l)) * sqrt(var) * Psifunc(abs(d(l+1))/sqrt(var));      
       end
       kgifinal = exp(sum(temp))/exp(parameters.c(i)*optbeta/parameters.P); 
    end
end
